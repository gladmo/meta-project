 ...
 */
export function cleanDocSiteOutput(siteRoot, outDir) {
 ...
 */
export function docSiteBuildOptions(siteRoot, mpa) {
  const root = resolve(siteRoot)
  return {
    ...mpa ? { mpa: 'true' } : {},
...
async function buildDocSite(siteRoot, mpa) {
  const root = resolve(siteRoot)
  await build(root, docSiteBuildOptions(root, mpa))
}

function parseMpa(args) {
